...
class CredentialManager:
    ...
    async def initialize(self, default_auth_mode: str = "proxy_key") -> None:
        ...
        logger.info(
            f"Auth config loaded: mode={self._config.auth_mode.value}, "
            f"validate={self._config.validate_credentials}"
        )
    ...
    async def _call_count_tokens(self, api_key: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 401:
                return False
            # Other errors (rate limit, server error) - don't cache, re-raise
            ...
            # Network error - fail closed (reject)
            return False
