...
async def resolve_anthropic_client(
    ...
    if explicit_key is not None:
        ...
        if credential_manager:
            await credential_manager.record_credential_type("client_api_key")
    ...
    is_passthrough_mode = credential_manager is not None and credential_manager.config.auth_mode == AuthMode.PASSTHROUGH
    use_passthrough = not matches_proxy_key or is_passthrough_mode
    if use_passthrough:
        if is_bearer and not is_anthropic_api_key(token):
            if credential_manager:
                await credential_manager.record_credential_type("oauth")
    ...
    if credential_manager:
        await credential_manager.record_credential_type("proxy_key_fallback")
    return base_client
