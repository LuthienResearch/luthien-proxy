...
function renderPolicyCard(p, container) {
    ...
    // Badges
    if (p.catalog_badges && p.catalog_badges.length > 0) {
        for (const badge of p.catalog_badges) {
...
function renderProposedSingle(content) {
    ...
    // Judge prompt — pulled from the policy's actual config (example_config.instructions),
    // so this is exactly what the judge LLM sees at runtime. No drift between this label
    // and the runtime prompt.
    const judgeInstructions = (p.example_config && typeof p.example_config.instructions === 'string')
        ? p.example_config.instructions
        : '';
    ...
    // Policy preview (UI hint only — production output may differ for LLM-judge or
    // templated runtime alerts; see UIMetadata.ui_policy_preview docstring).
    if (p.ui_policy_preview) {
        ...
        alertLabel.textContent = 'Policy preview (production output may differ)';
        const alertText = document.createElement('div');
        alertText.className = 'alert-preview-text';
        alertText.textContent = p.ui_policy_preview;
