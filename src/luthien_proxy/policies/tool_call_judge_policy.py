...
class ToolCallJudgePolicy(BasePolicy, OpenAIPolicyInterface, AnthropicPolicyInterface):
    ...
    async def _handle_anthropic_content_block_stop(
        ...
        if blocked_result is not None:
            ...
            text_block = TextBlock.model_construct(type="text", text="")
            start_event = RawContentBlockStartEvent.model_construct(
                type="content_block_start", index=index, content_block=text_block
            )
            text_delta = TextDelta.model_construct(type="text_delta", text=blocked_message)
