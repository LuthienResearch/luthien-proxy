...
def test_ensure_env_sentry_enabled(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    _ensure_env(str(repo), "sk-key", "admin-key", sentry_enabled=True, sentry_dsn="https://test@sentry.io/1")
    env_content = (repo / ".env").read_text()
    assert "SENTRY_ENABLED=true" in env_content
    assert "SENTRY_DSN=https://test@sentry.io/1" in env_content


def test_ensure_env_sentry_disabled_no_dsn(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    _ensure_env(str(repo), "sk-key", "admin-key", sentry_enabled=False)
    env_content = (repo / ".env").read_text()
    assert "SENTRY_ENABLED=false" in env_content
