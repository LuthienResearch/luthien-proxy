...
async function loadRecentCIDs() {
  ...
  try {
    const items = await fetchJSON('/api/hooks/recent_cids?limit=50');
    if (!Array.isArray(items) || items.length === 0) { list.textContent = 'No recent correlation IDs'; return; }
    list.textContent = '';
    for (const it of items) {
      const row = el('div', {class: 'cid-item'});
      const left = el('div');
      left.appendChild(el('div', {class:'cid', text: it.correlation_id}));
      ...
      btn.addEventListener('click', () => loadTrace(it.correlation_id));
      row.append(left, btn);
      row.addEventListener('click', (e)=>{ if (e.target!==btn) loadTrace(it.correlation_id); });
...
function renderTimeline(data) {
  ...
  t.textContent = '';
  if (!data || !Array.isArray(data.entries) || data.entries.length === 0) { t.textContent = 'No entries for this correlation_id'; return; }
...
async function loadTrace(cid) {
  ...
  input.value = cid;
  const data = await fetchJSON(`/api/hooks/trace?cid=${encodeURIComponent(cid)}`);
  renderTimeline(data);
}
