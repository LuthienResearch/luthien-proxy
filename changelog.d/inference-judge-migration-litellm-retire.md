...
**Migrate judges to InferenceProvider, retire LiteLLM**:
  ...
  - **Deployment note**: `boto3` (used by the opt-in S3 conversation archiver) previously arrived transitively via `litellm[proxy]`. It is now an optional extra — operators using S3 archiving must install `luthien-proxy[archive]`.
