...
- [ ] **MultiParallelPolicy assumes sub-policies have `on_anthropic_response`/`on_anthropic_stream_event`** - It calls these methods via `type: ignore[attr-defined]`. TextModifierPolicy-based policies (e.g., AllCapsPolicy) don't naturally expose them since they handle Anthropic via `run_anthropic`. AllCapsPolicy has compatibility shims for now. Fix by having MultiParallelPolicy use `run_anthropic` for the Anthropic path.
