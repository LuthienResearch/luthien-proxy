...
- [ ] **Add guard test: assert Python `_SKIP_MESSAGES` matches SQL `NOT IN` list** - The preview extraction in `history/service.py` has two layers of probe filtering (Python set + SQL clause) that must stay in sync. Parse the SQL string in the test to extract the `NOT IN` values and assert they match the Python `_SKIP_MESSAGES` set. Pattern from PR #222's migration guard test. COE from PR #277, 2026-03-05.
