...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-passthrough",
        action="store_true",
        help="Use proxy API key instead of ANTHROPIC_API_KEY (default: passthrough enabled)",
    )
...
async def run_overseer(args: argparse.Namespace) -> None:
    ...
    if args.no_passthrough:
        ...
        if not api_key:
            logger.error("ANTHROPIC_API_KEY not set. Use --no-passthrough to use proxy key instead.")
            sys.exit(1)
