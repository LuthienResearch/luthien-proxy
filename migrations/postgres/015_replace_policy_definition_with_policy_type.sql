...
COMMENT ON TABLE policy_type IS 'Registry of available policy types (built-in only in v1). module_path is the stable identifier for built-in types.';
...
COMMENT ON COLUMN policy_type.module_path IS 'Stable module path (e.g., "luthien_proxy.policies.noop_policy:NoOpPolicy"). Unique per built-in. Null for non-built-in types.';
