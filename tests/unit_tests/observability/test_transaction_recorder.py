...
class TestDefaultTransactionRecorder:
    ...
    async def test_record_request_emits_event(self):
        """record_request emits event via global emitter."""
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            recorder = DefaultTransactionRecorder(transaction_id="test-txn-123")
            ...
            # Verify record_event_sync called with correct data
            mock_emit.assert_called_once()
            call_args = mock_emit.call_args
            assert call_args[0][0] == "test-txn-123"  # transaction_id
            assert call_args[0][1] == "transaction.request_recorded"  # event_type
            data = call_args[0][2]
    ...
    async def test_ingress_chunks_within_limit_are_included(self):
        ...
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            recorder = DefaultTransactionRecorder(transaction_id="test-txn", max_chunks_queued=3)
    ...
    async def test_ingress_chunks_beyond_limit_are_truncated(self):
        ...
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            recorder = DefaultTransactionRecorder(transaction_id="test-txn", max_chunks_queued=2)
            ...
            finalize_call = mock_emit.call_args
            data = finalize_call[0][2]
    ...
    def test_ingress_truncation_emits_event(self):
        ...
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            ...
            mock_emit.assert_called_once()
            call_args = mock_emit.call_args
            assert call_args[0][1] == "transaction.recorder.ingress_truncated"
            assert "max_chunks_queued_exceeded" in call_args[0][2]["reason"]
    ...
    def test_egress_truncation_emits_event(self):
        ...
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            ...
            mock_emit.assert_called_once()
            call_args = mock_emit.call_args
            assert call_args[0][1] == "transaction.recorder.egress_truncated"
            assert "max_chunks_queued_exceeded" in call_args[0][2]["reason"]
    ...
    async def test_finalize_streaming_response_reconstructs_and_emits(self):
        ...
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            ...
            mock_emit.assert_called_once()
            call_args = mock_emit.call_args
            assert call_args[0][1] == "transaction.streaming_response_recorded"
            data = call_args[0][2]
    ...
    async def test_record_response_emits_responses(self):
        ...
        with patch("luthien_proxy.observability.transaction_recorder.record_event_sync") as mock_emit:
            ...
            mock_emit.assert_called_once()
            call_args = mock_emit.call_args
            assert call_args[0][1] == "transaction.non_streaming_response_recorded"
            data = call_args[0][2]
