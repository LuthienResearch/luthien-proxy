...
@pytest.mark.asyncio
async def test_open_proxy_closed_no_client_api_key():
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="any-token")
    with pytest.raises(HTTPException) as exc_info:
        await verify_strict_client_key(credentials=credentials, api_key=None)
    assert exc_info.value.status_code == 401


@pytest.mark.asyncio
async def test_open_proxy_closed_wrong_token():
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="wrong-key")
    with pytest.raises(HTTPException) as exc_info:
        await verify_strict_client_key(credentials=credentials, api_key="correct-key")
    assert exc_info.value.status_code == 401


@pytest.mark.asyncio
async def test_valid_client_key_accepted():
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="correct-key")
    result = await verify_strict_client_key(credentials=credentials, api_key="correct-key")
...
@pytest.mark.asyncio
async def test_strict_client_key_rejects_when_env_unset():
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="any-token")
...
@pytest.mark.asyncio
async def test_strict_client_key_rejects_wrong_token():
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="sk-wrong-key")

    with pytest.raises(HTTPException) as exc_info:
        await verify_strict_client_key(credentials=credentials, api_key="sk-correct-key")

    assert exc_info.value.status_code == 401


@pytest.mark.asyncio
async def test_strict_client_key_accepts_matching_token():
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="sk-correct-key")

    result = await verify_strict_client_key(credentials=credentials, api_key="sk-correct-key")
