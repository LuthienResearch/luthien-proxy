...
async def _get_session(
    client: httpx.AsyncClient, gateway_url: str, admin_headers: dict, session_uuid: str
) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_session_stored_after_request(
    mock_anthropic: MockAnthropicServer, gateway_healthy, gateway_url: str, auth_headers: dict, admin_headers: dict
):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await _send_message(client, gateway_url, auth_headers, "hello", session_uuid=session_uuid)
        assert resp.status_code == 200

        # Give the gateway time to persist the conversation event
        await asyncio.sleep(1.0)

        history_resp = await _get_session(client, gateway_url, admin_headers, session_uuid)
...
@pytest.mark.asyncio
async def test_session_contains_conversation_turn(
    mock_anthropic: MockAnthropicServer, gateway_healthy, gateway_url: str, auth_headers: dict, admin_headers: dict
):
...
@pytest.mark.asyncio
async def test_multiple_turns_in_same_session(
    mock_anthropic: MockAnthropicServer, gateway_healthy, gateway_url: str, auth_headers: dict, admin_headers: dict
):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp1 = await _send_message(client, gateway_url, auth_headers, "first message", session_uuid=session_uuid)
        assert resp1.status_code == 200

        resp2 = await _send_message(client, gateway_url, auth_headers, "second message", session_uuid=session_uuid)
...
@pytest.mark.asyncio
async def test_request_without_session_id_not_tracked(
    mock_anthropic: MockAnthropicServer, gateway_healthy, gateway_url: str, auth_headers: dict, admin_headers: dict
):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await _send_message(client, gateway_url, auth_headers, "hello without session")
        assert resp.status_code == 200

        await asyncio.sleep(0.5)

        # Query with a UUID we never sent — should be 404
        history_resp = await _get_session(client, gateway_url, admin_headers, random_uuid)
...
async def test_different_sessions_are_independent(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp_a = await _send_message(client, gateway_url, auth_headers, "message for A", session_uuid=session_a)
        assert resp_a.status_code == 200

        resp_b = await _send_message(client, gateway_url, auth_headers, "message for B", session_uuid=session_b)
        assert resp_b.status_code == 200

        await asyncio.sleep(1.0)

        history_a = await _get_session(client, gateway_url, admin_headers, session_a)
        history_b = await _get_session(client, gateway_url, admin_headers, session_b)
...
@pytest.mark.asyncio
async def test_session_list_includes_recent_session(
    mock_anthropic: MockAnthropicServer, gateway_healthy, gateway_url: str, auth_headers: dict, admin_headers: dict
):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await _send_message(client, gateway_url, auth_headers, "list me", session_uuid=session_uuid)
        assert resp.status_code == 200
...
async def test_session_list_pagination(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp_a = await _send_message(
            client, gateway_url, auth_headers, "first paginated message", session_uuid=session_a
        )
        assert resp_a.status_code == 200

        resp_b = await _send_message(
            client, gateway_url, auth_headers, "second paginated message", session_uuid=session_b
        )
        assert resp_b.status_code == 200
...
@pytest.mark.asyncio
async def test_session_export_returns_markdown(
    mock_anthropic: MockAnthropicServer, gateway_healthy, gateway_url: str, auth_headers: dict, admin_headers: dict
):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await _send_message(client, gateway_url, auth_headers, "export this", session_uuid=session_uuid)
        assert resp.status_code == 200
...
@pytest.mark.asyncio
async def test_malformed_session_id_returns_404(gateway_healthy, gateway_url: str, admin_headers: dict):
