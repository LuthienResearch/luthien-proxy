...
**Run:**
```bash
uv run pytest -m mock_e2e tests/luthien_proxy/e2e_tests/ -x -v
```
...
**Run:**
```bash
uv run pytest -m "e2e and not mock_e2e and not sqlite_e2e" tests/luthien_proxy/e2e_tests/ -x -v
```
...
**Run:**
```bash
uv run pytest -m sqlite_e2e tests/luthien_proxy/e2e_tests/sqlite/ -x -v
```
...
docker compose -f docker-compose.yaml -f docker-compose.mock-bridge.yaml up -d
uv run pytest -m mock_e2e tests/luthien_proxy/e2e_tests/ -x -v --timeout=120

# 3. Real e2e tests (standard config)
docker compose up -d gateway
uv run pytest -m "e2e and not mock_e2e and not sqlite_e2e" tests/luthien_proxy/e2e_tests/ -x -v --timeout=120 \
...
uv run pytest -m e2e tests/luthien_proxy/e2e_tests/test_policy_composition.py -v --timeout=120
...
E2E_GATEWAY_URL=http://localhost:8000 uv run pytest -m e2e tests/luthien_proxy/e2e_tests/test_request_logging.py -v --timeout=120
