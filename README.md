...
Luthien has three independent authentication layers. The default `luthien onboard` flow only sets `ADMIN_API_KEY`; the other two keys are optional and control opt-in behaviors.

| Env var | What it controls | When to set it | Default behavior if unset |
|---|---|---|---|
| `ADMIN_API_KEY` | Admin dashboard + admin API (History, Policy Config, `/api/admin/*`) | Set by `luthien onboard` automatically | Required for remote access. Localhost requests bypass auth by default (`LOCALHOST_AUTH_BYPASS=true`). |
| `PROXY_API_KEY` | Gateway ingress: the key clients present to talk to `/v1/messages` | Only if you want to require a shared gateway key for clients | Clients authenticate with their own Anthropic credentials (OAuth token or `x-api-key`), which are passed through to Anthropic. |
| `ANTHROPIC_API_KEY` | Gateway egress: server-side key used when `PROXY_API_KEY` auth succeeds | Only if you want all traffic billed to one Anthropic account | Each client's own credentials are forwarded upstream. |
...
After `luthien onboard`, the gateway runs in `AUTH_MODE=both`, which means:

1. Clients can pass their own Anthropic OAuth token or API key and it is forwarded upstream (**passthrough** — no gateway key needed).
2. Or, if you set `PROXY_API_KEY`, clients that present it authenticate against the gateway and get billed to `ANTHROPIC_API_KEY` (if set) or fail with 500 (if not).

You only need to touch `PROXY_API_KEY` / `ANTHROPIC_API_KEY` if you want a shared server-managed credential for multiple clients. For a single Claude Pro/Max user on localhost, neither is required.
...
# Optional — only needed if you want a shared gateway key for clients
PROXY_API_KEY=sk-luthien-dev-key

# Optional — only needed if `PROXY_API_KEY`-authenticated traffic should be billed
# to a server-side Anthropic account instead of the client's own credentials
